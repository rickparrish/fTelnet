...
class CrtFonts {
    ...
    public static __ctor() {
        ...
        for (var key in this._Fonts) {
            if (this._Fonts.hasOwnProperty(key)) {
                this._Fonts[key].sort((a: Point, b: Point): number => {
                    if (b.x - a.x === 0) {
                        return b.y - a.y;
                    } else {
                        return b.x - a.x;
    ...
    public static GetRemoteUrl(font: string, width: number, height: number): string {
        var PngUrl: string = '//embed-v2.ftelnet.ca/ftelnet/fonts/' + font + '_' + width.toString(10) + 'x' + height.toString(10) + '.png';
        return PngUrl;
    }
