...
else{this.onmousereport.trigger('\x1B[M '+me.button.toString()+'!'+(this._MouseDownPoint.x-1).toString()+'!'+(this._MouseDownPoint.y-1).toString());}}};Crt.prototype.OnMouseMove=function(me){if(typeof this._MouseDownPoint==='undefined'){return;}
...
if(this._NextByte===0){if(this._Connection.bytesAvailable===0){if(this._ShouldSendC&&((new Date()).getTime()-this._LastCTime>3000)){try{this._Connection.writeByte(this.CAPC);this._Connection.flush();}
catch(ioe1){this.HandleIOError(ioe1);return;}
this._LastCTime=new Date().getTime();}
return;}
else{try{this._NextByte=this._Connection.readUnsignedByte();}
catch(ioe2){this.HandleIOError(ioe2);return;}}}
this.DebugLog('Next byte is '+this._NextByte.toString());switch(this._NextByte){case this.CAN:this.CleanUp('Sender requested abort');break;case this.SOH:case this.STX:this._ShouldSendC=false;var BlockSize=(this._NextByte===this.STX)?1024:128;var neededBytes=1+1+BlockSize+1+1;if(this._Connection.bytesAvailable<neededBytes){this.DebugLog('Need '+neededBytes.toString()+' but only have '+this._Connection.bytesAvailable.toString()+' available');return;}
...
this._File=new FileRecord(FileName,FileSize);this.lblFileCount.Text='Receiving file '+(this._Files.length+1).toString();this.lblFileName.Text='File Name: '+FileName;this.lblFileSize.Text='File Size: '+StringUtils.AddCommas(FileSize)+' bytes';this.lblFileReceived.Text='File Recv: 0 bytes';this.pbFileReceived.Value=0;this.pbFileReceived.Maximum=FileSize;try{this._Connection.writeByte(this.ACK);this._Connection.writeByte(this.CAPC);this._Connection.flush();}
catch(ioe3){this.HandleIOError(ioe3);return;}}
else{var BytesToWrite=Math.min(BlockSize,this._File.size-this._File.data.length);this._File.data.writeBytes(Packet,0,BytesToWrite);this._TotalBytesReceived+=BytesToWrite;this.lblFileReceived.Text='File Recv: '+StringUtils.AddCommas(this._File.data.length)+' bytes';this.pbFileReceived.Value=this._File.data.length;this.lblTotalReceived.Text='Total Recv: '+StringUtils.AddCommas(this._TotalBytesReceived)+' bytes';try{this._Connection.writeByte(this.ACK);this._Connection.flush();}
catch(ioe4){this.HandleIOError(ioe4);return;}}
break;case this.EOT:this._ShouldSendC=true;try{this._Connection.writeByte(this.ACK);this._Connection.writeByte(this.CAPC);this._Connection.flush();}
...
var FileBlob=new Blob([Buffer],{type:'application/octet-binary'});saveAs(FileBlob,this._Files[index].name);};return YModemReceive;}());var YModemSend=(function(){function YModemSend(crt,connection){this.ontransfercomplete=new TypedEvent();this.SOH=0x01;this.STX=0x02;this.EOT=0x04;this.ACK=0x06;this.NAK=0x15;this.CAN=0x18;this.SUB=0x1A;this.CAPC='C'.charCodeAt(0);this.CAPG='G'.charCodeAt(0);this._Block=0;this._Debug=(window.location.hash.indexOf('ymodemdebug=1')>=0);this._EOTCount=0;this._FileBytesSent=0;this._FileCount=0;this._Files=[];this._State=YModemSendState.WaitingForHeaderRequest;this._TotalBytes=0;this._TotalBytesSent=0;this._Crt=crt;this._Connection=connection;}
YModemSend.prototype.Cancel=function(reason){this.DebugLog('Cancel at state '+this._State+': '+reason);try{this._Connection.writeByte(this.CAN);this._Connection.writeByte(this.CAN);this._Connection.writeByte(this.CAN);this._Connection.writeByte(this.CAN);this._Connection.writeByte(this.CAN);this._Connection.writeString('\b\b\b\b\b     \b\b\b\b\b');}
catch(ioe1){this.HandleIOError(ioe1);return;}
try{this._Connection.readString();}
catch(ioe2){this.HandleIOError(ioe2);return;}
this.CleanUp('Cancelling ('+reason+')');};YModemSend.prototype.CleanUp=function(message){var _this=this;clearInterval(this._Timer);this.lblStatus.Text='Status: '+message;setTimeout(function(){_this.Dispatch();},3000);};YModemSend.prototype.DebugLog=function(message){if(this._Debug){console.log(message);}};YModemSend.prototype.Dispatch=function(){this.pnlMain.Hide();this._Crt.ShowCursor();this.ontransfercomplete.trigger();};YModemSend.prototype.HandleIOError=function(ioe){this.DebugLog('I/O Error: '+ioe);if(this._Connection.connected){this.CleanUp('Unhandled I/O error');}
...
if(this._Connection.bytesAvailable===0){return;}
var B=0;switch(this._State){case YModemSendState.WaitingForHeaderRequest:try{B=this._Connection.readUnsignedByte();}
catch(ioe1){this.HandleIOError(ioe1);return;}
if(B!==this.CAPC){if(B===this.CAPG){this.Cancel('Use plain YMODEM, not YMODEM-G');}
else{this.Cancel('Expecting C got '+B.toString()+' (State='+this._State+')');}
return;}
try{this._Connection.readString();}
catch(ioe2){this.HandleIOError(ioe2);return;}
...
this._File=NextFile;this.lblFileCount.Text='Sending file '+(this._FileCount-this._Files.length).toString()+' of '+this._FileCount.toString();this.lblFileName.Text='File Name: '+this._File.name;this.lblFileSize.Text='File Size: '+StringUtils.AddCommas(this._File.size)+' bytes';this.lblFileSent.Text='File Sent: 0 bytes';this.pbFileSent.Value=0;this.pbFileSent.Maximum=this._File.size;this.SendHeaderBlock();this._Block=1;this._EOTCount=0;this._FileBytesSent=0;this.SetState(YModemSendState.WaitingForHeaderAck);return;case YModemSendState.WaitingForHeaderAck:try{B=this._Connection.readUnsignedByte();}
catch(ioe3){this.HandleIOError(ioe3);return;}
if((B!==this.ACK)&&(B!==this.CAPC)){this.Cancel('Expecting ACK or C got '+B.toString()+' (State='+this._State+')');return;}
if(B===this.ACK){this.SetState(YModemSendState.WaitingForFileRequest);}
else if(B===this.CAPC){this.DebugLog('Got C, sending a data block');this.SendDataBlock();}
return;case YModemSendState.WaitingForFileRequest:try{B=this._Connection.readUnsignedByte();}
catch(ioe4){this.HandleIOError(ioe4);return;}
if(B!==this.CAPC){this.Cancel('Expecting C got '+B.toString()+' (State='+this._State+')');return;}
this.DebugLog('Got C, sending a data block');this.SendDataBlock();return;case YModemSendState.WaitingForBlockAck:try{B=this._Connection.readUnsignedByte();}
catch(ioe4){this.HandleIOError(ioe4);return;}
if(B===this.CAPC){this.DebugLog('Got unexpected C while waiting for block ack, ignoring');return;}
if((B!==this.ACK)&&(B!==this.NAK)){this.Cancel('Expecting (N)ACK got '+B.toString()+' (State='+this._State+')');return;}
this.DebugLog('Got ACK, sending a data block');this.SendDataBlock();return;case YModemSendState.WaitingForFileAck:try{B=this._Connection.readUnsignedByte();}
catch(ioe5){this.HandleIOError(ioe5);return;}
if((B!==this.ACK)&&(B!==this.NAK)){this.Cancel('Expecting (N)ACK got '+B.toString()+' (State='+this._State+')');return;}
if(B===this.ACK){this.SetState(YModemSendState.WaitingForHeaderRequest);}
else if(B===this.NAK){this.DebugLog('Got NAK, re-sending EOT');this.SendEOT();}
return;}};YModemSend.prototype.SendDataBlock=function(){if(this.SendDataBlocks(1)){this.SetState(YModemSendState.WaitingForFileAck);}
else{this.SetState(YModemSendState.WaitingForBlockAck);}};YModemSend.prototype.SendDataBlocks=function(blocks){this.DebugLog('SendDataBlocks sending '+blocks.toString()+' data block(s)');for(var loop=0;loop<blocks;loop++){var BytesToRead=Math.min(1024,this._File.data.bytesAvailable);if(BytesToRead===0){this.SendEOT();return true;}
...
return false;};YModemSend.prototype.SendEmptyHeaderBlock=function(){this.DebugLog('SendEmptyHeaderBlock');var Packet=new ByteArray();for(var i=0;i<128;i++){Packet.writeByte(0);}
...
catch(ioe){this.HandleIOError(ioe);return;}};YModemSend.prototype.SendEOT=function(){this.DebugLog('SendEOT');try{this._Connection.writeByte(this.EOT);this._Connection.flush();}
catch(ioe){this.HandleIOError(ioe);return;}
this._EOTCount++;};YModemSend.prototype.SendHeaderBlock=function(){this.DebugLog('SendHeaderBlock');var i=0;var Packet=new ByteArray();for(i=0;i<this._File.name.length;i++){Packet.writeByte(this._File.name.charCodeAt(i));}
...
return ViewPortSettings;}());var WriteMode;(function(WriteMode){WriteMode[WriteMode["Normal"]=0]="Normal";WriteMode[WriteMode["Copy"]=0]="Copy";WriteMode[WriteMode["XOR"]=1]="XOR";WriteMode[WriteMode["Or"]=2]="Or";WriteMode[WriteMode["And"]=3]="And";WriteMode[WriteMode["Not"]=4]="Not";})(WriteMode||(WriteMode={}));var ButtonStyle=(function(){function ButtonStyle(){this.width=0;this.height=0;this.orientation=0;this.flags=0;this.bevelsize=0;this.dfore=0;this.dback=0;this.bright=0;this.dark=0;this.surface=0;this.groupid=0;this.flags2=0;this.underlinecolour=0;this.cornercolour=0;}
...
if(this._Options.Emulation==='C64'){this._Options.Font='C64-Lower';this._Options.ScreenColumns=40;}
...
catch(e){console.log('failed to get proxy-servers.json: '+e);this._LoadingProxySettings=0;}};fTelnetClient.prototype.OnAnsiESC0c=function(){if(typeof this._Connection==='undefined'){return;}
if(!this._Connection.connected){return;}
