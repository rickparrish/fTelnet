...
this._Benchmarks[name].Start();return this._Benchmarks[name];};Benchmarks.Stop=function(name){this._Benchmarks[name].Stop();};Benchmarks.Log=function(){for(var i=0;i<this._Names.length;i++){var text='';text+=this._Names[i]+': '+this._Benchmarks[this._Names[i]].CumulativeElapsed;console.log(text);}};return Benchmarks;}());Benchmarks._Benchmarks={};Benchmarks._Names=[];var Benchmark=(function(){function Benchmark(){this._CumulativeElapsed=0;}
...
else{return undefined;}},enumerable:true,configurable:true});Benchmark.prototype.Reset=function(){this._CumulativeElapsed=0;};Benchmark.prototype.Start=function(){this._StartTime=performance.now();this._StopTime=0;};Benchmark.prototype.Stop=function(){if(this._StartTime>0){this._StopTime=performance.now();this._CumulativeElapsed+=this.Elapsed;}};return Benchmark;}());var ByteArray=(function(){function ByteArray(){this._Bytes=[];this._Length=0;this._Position=0;}
...
for(var i=0;i<TextLength;i++){var BGetChar=Benchmarks.Start('GetChar');var Char=this._Font.GetChar(CharCodes[i],charInfo);BGetChar.Stop();var BPutImage=Benchmarks.Start('PutImage');if(typeof Char!=='undefined'){if(this._UseModernScrollback){this._CanvasContext.putImageData(Char,(x-1+i)*this._Font.Width,(y-1+this._ScrollbackSize)*this._Font.Height);}
...
BPutImage.Stop();var BUpdateBuffer=Benchmarks.Start('UpdateBuffer');if(updateBuffer){var CharToUpdate=this._Buffer[y][x+i];CharToUpdate.Ch=Chars[i];CharToUpdate.Attr=charInfo.Attr;CharToUpdate.Blink=charInfo.Blink;CharToUpdate.Underline=charInfo.Underline;CharToUpdate.Reverse=charInfo.Reverse;}
...
BClearBottom.Stop();}
var BScrollUpdateBuffer=Benchmarks.Start('ScrollUpdateBuffer');if(updateBuffer){var NewRow;var X;var Y;if(!this._UseModernScrollback){for(Y=0;Y<count;Y++){NewRow=[];for(X=left;X<=right;X++){NewRow.push(new CharInfo(this._Buffer[Y+top][X].Ch,this._Buffer[Y+top][X].Attr,this._Buffer[Y+top][X].Blink,this._Buffer[Y+top][X].Underline,this._Buffer[Y+top][X].Reverse));}
...
for(Y=bottom;Y>(bottom-count);Y--){for(X=left;X<=right;X++){this._Buffer[Y][X].Ch=charInfo.Ch;this._Buffer[Y][X].Attr=charInfo.Attr;this._Buffer[Y][X].Blink=charInfo.Blink;this._Buffer[Y][X].Underline=charInfo.Underline;this._Buffer[Y][X].Reverse=charInfo.Reverse;}}}
