...
return KeyPressEvent;}());var RLoginCommand;(function(RLoginCommand){RLoginCommand[RLoginCommand["Cookie"]=255]="Cookie";RLoginCommand[RLoginCommand["S"]=115]="S";})(RLoginCommand||(RLoginCommand={}));if(!window.cordova){if(('WebSocket'in window)&&!navigator.userAgent.match('AppleWebKit/534.30')){}
...
var WebSocketProtocol=(document.location.protocol==='https:'?'wss':'ws');var WebSocketSupportsTypedArrays=(('Uint8Array'in window)&&('set'in Uint8Array.prototype));var WebSocketSupportsBinaryType=false;if(!window.cordova){WebSocketSupportsBinaryType=(WebSocketSupportsTypedArrays&&('binaryType'in WebSocket.prototype||!!(new WebSocket(WebSocketProtocol+'://.').binaryType)));}
var WebSocketConnection=(function(){function WebSocketConnection(){this.onclose=new TypedEvent();this.onconnect=new TypedEvent();this.ondata=new TypedEvent();this.onlocalecho=new TypedEvent();this.onioerror=new TypedEvent();this.onsecurityerror=new TypedEvent();this._WasConnected=false;this._LocalEcho=false;this._Protocol='plain';this._Proxied=false;this._CordovaSocketConnected=false;this._InputBuffer=new ByteArray();this._OutputBuffer=new ByteArray();}
Object.defineProperty(WebSocketConnection.prototype,"bytesAvailable",{get:function(){return this._InputBuffer.bytesAvailable;},enumerable:true,configurable:true});WebSocketConnection.prototype.close=function(){if(window.cordova){if(this._CordovaSocket){window.tlantic.plugins.socket.disconnect(function(){},function(){console.log('TODOX disconnect failed');},this._CordovaSocket);}}
...
this._WasConnected=false;if(window.cordova){this._Proxied=false;window.tlantic.plugins.socket.connect(function(connectionId){_this._CordovaSocket=connectionId;_this._CordovaSocketConnected=true;_this.OnSocketOpen();},function(){console.log("TODOX connect failed");},hostname,port);document.addEventListener(window.tlantic.plugins.socket.receiveHookName,function(ev){if(_this._CordovaSocket===ev.metadata.connection.id){_this.OnCordovaSocketData(ev.metadata.data);}});}
...
this._WasConnected=false;};WebSocketConnection.prototype.OnSocketError=function(e){this.onioerror.trigger(e);};WebSocketConnection.prototype.OnSocketOpen=function(){if(!window.cordova){if(this._WebSocket.protocol){this._Protocol=this._WebSocket.protocol;}
...
this._Crt.PushKeyDown(0,KeyCode,this._CtrlPressed,this._AltPressed,this._ShiftPressed);if(NeedReset){this._AltPressed=false;this._CtrlPressed=false;this._ShiftPressed=false;this.ReDrawSpecialKeys();}}};VirtualKeyboard.prototype.OnTouchStart=function(){if(this._SupportsTouchEvents){return;}
