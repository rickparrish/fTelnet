...
else{this._TerminalTypeIndex=0;}};TelnetConnection.prototype.HandleTerminalLocationNumber=function(){var _this=this;if(this._SendLocation){var xhr=new XMLHttpRequest();xhr.open('get','https://text.ipv4.wtfismyip.com/',true);xhr.onload=function(){var status=xhr.status;if(status===200){_this.SendWill(TelnetOption.TerminalLocationNumber);_this.SendSubnegotiate(TelnetOption.TerminalLocationNumber);var SixtyFourBits=[];SixtyFourBits.push(0);var octets=xhr.responseText.split('.');SixtyFourBits.push(parseInt(octets[0],10)&0xFF);SixtyFourBits.push(parseInt(octets[1],10)&0xFF);SixtyFourBits.push(parseInt(octets[2],10)&0xFF);SixtyFourBits.push(parseInt(octets[3],10)&0xFF);SixtyFourBits.push(0xFF);SixtyFourBits.push(0xFF);SixtyFourBits.push(0xFF);SixtyFourBits.push(0xFF);var ToSendBytes=[];for(var i=0;i<SixtyFourBits.length;i++){ToSendBytes.push(SixtyFourBits[i]);if(SixtyFourBits[i]===TelnetCommand.IAC){ToSendBytes.push(TelnetCommand.IAC);}}
_this.Send(ToSendBytes);_this.SendSubnegotiateEnd();}
