...
this.onfontchange.trigger();};Crt.prototype.OnKeyDown=function(ke){if(!window.cordova){if((ke.target instanceof HTMLInputElement)||(ke.target instanceof HTMLTextAreaElement)){if(ke.target.id!=='fTelnetAriaInput'){return;}}}
...
this._KeyBuf.push(new KeyPressEvent(ke,keyString));if((keyString)||(ke.ctrlKey)){ke.preventDefault();this.onkeypressed.trigger();}};Crt.prototype.OnKeyPress=function(ke){if(!window.cordova){if((ke.target instanceof HTMLInputElement)||(ke.target instanceof HTMLTextAreaElement)){if(ke.target.id!=='fTelnetAriaInput'){return;}}}
...
var newAriaText=this._AriaText;for(var i=0;i<text.length;i++){var ch=text.charCodeAt(i);if((ch==10)||(ch==13)||((ch>=32)&&(ch<=126))){newAriaText+=text[i];}}
if(this._AriaText!=newAriaText){this._AriaText=newAriaText;this._Canvas.innerText=this._AriaText;}};Crt.prototype.WriteASCII=function(text){if(typeof text==='undefined'){text='';}
...
this._WasConnected=true;this.onconnect.trigger();};WebSocketConnection.prototype.OnWebSocketMessage=function(e){if(this._LogIO){console.log('Incoming '+(typeof e.data==='string'?'text':'binary')+' message');}
...
console.log('SEND: '+DebugLine);}};Object.defineProperty(WebSocketConnection.prototype,"SendLocation",{set:function(value){this._SendLocation=value;},enumerable:true,configurable:true});WebSocketConnection.prototype.writeByte=function(value){this._OutputBuffer.writeByte(value);};WebSocketConnection.prototype.writeBytes=function(bytes,offset,length){this._OutputBuffer.writeBytes(bytes,offset,length);};WebSocketConnection.prototype.writeShort=function(value){this._OutputBuffer.writeShort(value);};WebSocketConnection.prototype.writeString=function(text){this._OutputBuffer.writeString(text);this.flush();};return WebSocketConnection;}());var __extends=(this&&this.__extends)||(function(){var extendStatics=function(d,b){extendStatics=Object.setPrototypeOf||({__proto__:[]}instanceof Array&&function(d,b){d.__proto__=b;})||function(d,b){for(var p in b)if(Object.prototype.hasOwnProperty.call(b,p))d[p]=b[p];};return extendStatics(d,b);};return function(d,b){extendStatics(d,b);function __(){this.constructor=d;}
...
RLoginConnection.prototype.NegotiateInbound=function(data){var DebugLine="";while(data.bytesAvailable){var B=data.readUnsignedByte();if(this._LogIO){if(B>=32&&B<=126){DebugLine+=String.fromCharCode(B);}
else{DebugLine+='~'+B.toString(10);}}
if(this._NegotiationState===RLoginNegotiationState.Data){if(B===RLoginCommand.Cookie){this._NegotiationState=RLoginNegotiationState.Cookie1;}
...
else{this.SendWont(TelnetOption.Echo);}}},enumerable:true,configurable:true});TelnetConnection.prototype.NegotiateInbound=function(data){var DebugLine="";while(data.bytesAvailable){var B=data.readUnsignedByte();if(this._LogIO){if(B>=32&&B<=126){DebugLine+=String.fromCharCode(B);}
else{DebugLine+='~'+B.toString(10);}}
...
this._Timer=setInterval(function(){_this.OnTimer();},250);this._UploadInput=document.createElement('input');this._UploadInput.type='file';this._UploadInput.className='fTelnetUpload';this._UploadInput.onchange=function(){_this.OnUploadFileSelected();};this._UploadInput.style.display='none';this._fTelnetContainer.appendChild(this._UploadInput);if(this._Options.AriaInput){this._AriaInputWrapper=document.createElement('div');this._AriaInputWrapper.className='fTelnetAriaInputWrapper';this._fTelnetContainer.appendChild(this._AriaInputWrapper);this._AriaInput=document.createElement('input');this._AriaInput.id='fTelnetAriaInput';this._AriaInput.placeholder='Focus me to enter ARIA application mode';this._AriaInput.type='text';this._AriaInput.setAttribute('role','application');this._AriaInputWrapper.appendChild(this._AriaInput);}}
...
if(!this._Connection.connected){return;}
this._Connection.writeString('RIPSCRIP015400');}};fTelnetClient.prototype.OnAnsiRIPDisable=function(){};fTelnetClient.prototype.OnAnsiRIPEnable=function(){};fTelnetClient.prototype.OnConnectionClose=function(){if(typeof this._AriaInput!=='undefined'){this._AriaInput.blur();}
this._ConnectButton.innerHTML='Reconnect';this._ConnectButton.style.display='inline';this._StatusBarLabel.innerHTML='Disconnected from '+this._Options.Hostname+':'+this._Options.Port;this._StatusBar.style.backgroundColor='red';this._ClientContainer.style.opacity='0.5';};fTelnetClient.prototype.OnConnectionConnect=function(){if(typeof this._AriaInput!=='undefined'){this._AriaInput.focus();}
this._Crt.ClrScr();if(this._Options.ProxyHostname===''){this._StatusBarLabel.innerHTML='Connected to '+this._Options.Hostname+':'+this._Options.Port;this._StatusBar.style.backgroundColor='blue';this._ClientContainer.style.opacity='1.0';}
